...
def display_page(conversations):
    st.write("Play all")
    st.audio(
...
tab1, tab2 = st.tabs(["In restaurant", "Fussy restaurant client"])
with tab1:
    st.image("img/restaurant_1.png", use_column_width='always')
    st.header("In restaurant")
    display_page(load_conversation_data("conv_data/restaurant_1.json"))

with tab2:
    st.image("img/restaurant_2.png", use_column_width='always')
    st.header("Fussy restaurant client")
    display_page(load_conversation_data("conv_data/restaurant_2.json"))
